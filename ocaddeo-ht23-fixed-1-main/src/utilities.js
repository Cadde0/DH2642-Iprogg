...
function compareIngredientsCB(ingredientA, ingredientB){
    ...
    if(ingredientA.aisle > ingredientB.aisle) {
        ...
        return -1;
    } else {    //Aisles are the same, Compare by name:
    ...
    //see Array.sort() documentation Array.sort()
...
function sortDishes(dishes){
    return [...dishes].sort(compareTypeCB);
}
...
function menuPrice(dishesArray){
    ...
    //Dish Price Property = pricePerServing

    function pricesCB(dish) {
        return dish.pricePerServing;
    }


    function totPriceCB(total, start) {
        return total + start;
    }
    ...
    const pricesArray = dishesArray.map(pricesCB);
    const totalPrice = pricesArray.reduce(totPriceCB, 0);
    return totalPrice;
...
export {compareIngredientsCB, sortIngredients, isKnownTypeCB, dishType, 
    sortDishes, shoppingList, menuPrice/*TODO add more here! */};
